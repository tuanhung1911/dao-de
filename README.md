...
Mở file `.firebaserc` và sửa nội dung thành (thay `YOUR_FIREBASE_PROJECT_ID` bằng Project ID thực tế của bạn):
...
Tìm dòng `projectId` ở dưới cùng và sửa thành Project ID của bạn:

```yaml
          projectId: YOUR_FIREBASE_PROJECT_ID
```

Tìm dòng `firebaseServiceAccount` và đảm bảo tên biến bí mật là `FIREBASE_SERVICE_ACCOUNT`:
```yaml
...
5. Nhấn nút **New repository secret**.
   - **Name:** Điền tên là `FIREBASE_SERVICE_ACCOUNT` (khớp với file workflow).
